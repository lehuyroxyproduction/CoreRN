...
class FirstGuard extends React.Component {
  state = {
    title: '',
    name: '',
    phone_number: '',
  ...
  onNext = () => {
    ...
    if (title.trim() && name.trim() && phone_number && year_of_birth.trim()) {
      ...
      } else if (!Number(year_of_birth) || new Date().getFullYear() - year_of_birth < 18) {
        alert('Vui lòng nhập năm sinh hợp lệ, người thân phải từ 18 tuổi')
      } else {
        this.props.push({
          screen: 'RegisterSecondGuard',
          passProps: {
            guard: {
              title,
              name,
      ...
      <Container
        ...
        >
          ...
          <Text tiny center style={{ marginVertical: 24 }}>
            Người thân phải là người trong gia đình, trên 18 tuổi và có số điện thoại di động có thể liên hệ được
          </Text>
          ...
          <View
            ...
            <Touchable round={60} gradient color={Colors.coolGrey} onPress={this.onNext}>
